...
exports.signin = async (req, res, next) => {
    try {
        ...
        if (!systemAdmin) { return res.status(401).send({  message: "Invalid email or password" })}
...
exports.signup = async (req, res, next) => {
    try {
        ...
        // Saving the user
        const savedUser = await new systemAdminModel({ ...req.body, password: hashedPassword }).save();

        // Saving a user's token in the database
        const userInfo = await systemAdminModel.findOne({email: savedUser.email});
        await new adminTokenModel({ userId: userInfo._id, token: userInfo.generateAuthToken() }).save();
...
exports.update = (req, res, next) => {
    systemAdminModel.findByIdAndUpdate(req.query.id, req.body)
